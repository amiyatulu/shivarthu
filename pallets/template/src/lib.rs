...
pub mod pallet {
	use crate::types::{
		CitizenDetails, DepartmentDetails, ProfileFundInfo, SchellingType,
		StakeDetails
	};
	use schelling_game::types::{
		SumTreeName
	};
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn challenge_profile(origin:OriginFor<T>, citizenid: u128) -> DispatchResult {
            ...
            let key = SumTreeName::UniqueIdenfier{citizen_id: citizenid, name:"key1".as_bytes().to_vec() };
